...
static bool waitFor(Pred pred, unsigned int timeoutMs, unsigned int pollMs = 10) {
...
INode *MotorManager::getNode(int motorIdx) {
    ...
    if (motorIdx < 0 || motorIdx >= count) {
        throw std::out_of_range("motorIdx " + std::to_string(motorIdx) +
                                " is out of range (nodeCount=" +
                                std::to_string(count) + ")");
    }
...
int MotorManager::findAndOpenPorts() {
    ...
    try {
        ...
        std::cout << "[MotorManager] Found " << comHubPorts.size() << " SC Hub port(s).\n";

        // Register and open only the first hub (port 0)
        size_t portCount = std::min(comHubPorts.size(),
                                    static_cast<size_t>(NET_CONTROLLER_MAX));
...
int MotorManager::enableMotors() {
    ...
    try {
        ...
        bool allReady = waitFor([this]() {
            for (size_t i = 0; i < myPort->NodeCount(); ++i) {
                if (!myPort->Nodes(i).Motion.IsReady()) return false;
            }
            return true;
        }, 3000);
...
int MotorManager::homeMotors() {
    ...
    try {
        for (size_t i = 0; i < myPort->NodeCount(); ++i) {
            ...
            bool homed = waitFor([&node]() {
                return node.Motion.Homing.WasHomed();
            }, 30000);
...
int MotorManager::setSetpoint(int motorIdx, double setpoint) {
    ...
    if (motorIdx == 0) {
        ...
        std::cerr << "[MotorManager] setSetpoint: only motor indices 0 and 1 supported.\n";
        return -1;
    }
    return 0;
}
...
int MotorManager::goToSetpoint() {
    ...
    try {
        if (Motor1 && Motor1->Motion.IsReady()) {
            ...
            std::cout << "[MotorManager] Motor1 moving to " << motor1Setpoint << " counts.\n";
        }
        if (Motor2 && Motor2->Motion.IsReady()) {
            Motor2->Motion.MovePosnStart(motor2Setpoint, true /*absolute*/);
            std::cout << "[MotorManager] Motor2 moving to " << motor2Setpoint << " counts.\n";
        }
    } catch (mnErr &e) {
...
int MotorManager::isReady() {
    ...
    try {
        for (size_t i = 0; i < myPort->NodeCount(); ++i) {
            if (!myPort->Nodes(i).Motion.IsReady()) return 0;
        }
    } catch (mnErr &e) {
